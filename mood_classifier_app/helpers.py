...
def csvColumnToList(file, column_name):
    ...
    return data_frame[column_name].to_list()

def getMood(sentence, model, tokenizer):
...
def convert_dataset_to_dataframe(dataset):
    for i in dataset.take(1):
      dataset_feat = i[0].numpy()
      dataset_lab = i[1].numpy()

    dataset = pd.DataFrame([dataset_feat, dataset_lab]).T
